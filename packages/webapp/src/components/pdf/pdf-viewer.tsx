...
export const PdfViewer: React.FC<PdfViewerProps> = ({
  ...
  const drawWrapper = useCallback(
    (draw: DrawingFunction) => {
      ...
      if (
        !canvas.current ||
        !cleanCanvasDataUrl.current ||
        isWaitingForPageRender.current
      ) {
        return;
      }
  ...
  const pdfButtons = (
    <Inline>
      <Button label="prev" onClick={showPrevPage}>
        Prev
      </Button>
      <Button label="next" onClick={showNextPage}>
        Next
      </Button>
    </Inline>
  );
      ...
      <Grid container spacing={1}>
        <Grid item xs={12}>
          <Paper
            ...
            style={{
              width: "100%",
              backgroundColor: NORTHSTAR_COLORS.GREY_200,
            }}
          >
            <div className={styles.pdfViewerWrapper}>
              <TransformWrapper minScale={0.8} initialScale={1}>
                {({ zoomIn, zoomOut }) => (
                  <div>
                    <div style={{ width: "100%" }}>
                      <div
                    ...
                    <div
                      style={{
                        height: canvas.current
                          ? canvas.current.height / QUALITY_SCALE_FACTOR
                          : "70vh",
                      }}
                    >
                      <TransformComponent>
                        <div className={styles.pdfViewer}>
                          <Document
                            ...
                            loading={
                              <div
                                style={{
                                  transform: `scale(${QUALITY_SCALE_FACTOR})`,
                                  transformOrigin: "top left",
                                }}
                              >
                                <LoadingIndicator />
